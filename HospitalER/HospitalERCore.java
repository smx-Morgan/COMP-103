...
public class HospitalERCore{
    ...
    public void run(){
        ...
        while (running){         // each time step, check whether the simulation should pause.
            ...
            for(Patient patients: this.treatmentRoom){
                temp.add(patients);
            }
            for(Patient pa: temp){
                if(pa.completedCurrentTreatment()){
                    this.treatmentRoom.remove(pa);
                    UI.println(time+ ": Discharge: " + pa);
                    patientNum++;
                    if(pa.getPriority()==1){
                        patientNumpri++;
            ...
            if(this.treatmentRoom.size()<MAX_PATIENTS && !waitingRoom.isEmpty()){
