...
/* Code for XMUT103 - 2021T1, Assignment 1
C
 */
...
public class Sokoban {
    ...
    private Stack <ActionRecord> history = new Stack<>();
    private Stack <ActionRecord> redo = new Stack<>();
    /** 
    ...
    public void undo(){
        if(!history.isEmpty()){
        ActionRecord ac = history.pop();
        redo.push(ac);
        String op = opposite(ac.direction());
    ...
    public void domouse(String action , double x, double y){
        ...
        if(action == "relese"){
            ...
            for(int i = 0; cells[r][c]==cells[a][b];i++){
            if(cells[r+1][c].isEmptyShelf()){
                drawCell(workerPos);                   
                workerPos = workerPos.next("down"); 
                drawWorker();                          
            }else if(cells[r][c+1].isEmptyShelf()){
                drawCell(workerPos);                   
                workerPos = workerPos.next("right"); 
                drawWorker();  
            }else if(cells[r][c-1].isEmptyShelf()){
                drawCell(workerPos);                   
                workerPos = workerPos.next("left"); 
                drawWorker();  
            }else if(cells[r-1][c].isEmptyShelf()){
                drawCell(workerPos);                   
                workerPos = workerPos.next("up"); 
                drawWorker();
            }
        }
    }
