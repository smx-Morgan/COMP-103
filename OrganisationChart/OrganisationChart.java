...
/* Code for XMUT103 - 2021T1, Assignment 5
 * Name:
 * Username:
 * ID:
 */
...
public class OrganisationChart {
    ...
    private void drawTree(Position pos) {
        ...
        /*# YOUR CODE HERE */
        Set <Position> teamofPos = pos.getTeam();
        for(Position p : teamofPos){
            drawTree(p);
    ...
    private Position findPosition(double x, double y, Position pos){
        ...
        Set <Position> teamofPos = pos.getTeam();
        for(Position p : teamofPos){
            Position selectPos = findPosition(x,y,p);
            if(selectPos != null){
            return selectPos;
            }
        }
    ...
    private void movePosition(Position pos, Position target) {
        ...
        /*# YOUR CODE HERE */
        //判断target是否在pos里面
        if(pos.getManager()!=null&&inSubtree(pos,target)){
        return;
        }else{
    ...
    public void removePosition(Position pos){
        /*# YOUR CODE HERE */
        if(pos.getRole() == null&& !pos.isManager()){
    ...
    private boolean inSubtree(Position pos, Position subtree) {
        ...
        //if in the node return true else false
         
        if(pos == subtree){
        return false;
        }
        Set <Position> teamofSub = subtree.getTeam();
        for(Position p : teamofSub){
            boolean selectPos = inSubtreeRecursion(p,pos);
            if(selectPos){
            return selectPos;
            }
        }
        Set <Position> teamofPos = pos.getTeam();
        for(Position p : teamofPos){
            boolean selectPos = inSubtreeRecursion(subtree,p);
            if(selectPos){
            return selectPos;
            }
        }
    ...
    private boolean inSubtreeRecursion(Position pos, Position node){
        if(pos == node){
        return true;
        }
        Set <Position> teamofPos = pos.getTeam();
        for(Position p : teamofPos){
             inSubtreeRecursion(p,node);
